...
export const MDXComponents = {
  ...
  Spotify: ({ trackId, width = '100%', height = '152' }: { trackId: string; width?: string; height?: string }) => (
    <div className="my-6">
      <iframe
  ...
  YouTube: ({ videoId, title = 'YouTube video' }: { videoId: string; title?: string }) => (
  ...
  Instagram: ({ postId }: { postId: string }) => (
      ...
      >
        <div style={{ padding: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'row', marginBottom: '14px', alignItems: 'center' }}>
            <div>
              <div style={{ backgroundColor: '#F4F4F4', borderRadius: '50%', height: '12.5px', width: '12.5px', transform: 'translateX(0px) translateY(0px)' }}></div>
              <div style={{ backgroundColor: '#F4F4F4', height: '12.5px', transform: 'rotate(-45deg) translateX(3px) translateY(1px)', width: '12.5px', flexShrink: '0', marginRight: '14px', marginLeft: '2px' }}></div>
            ...
            <div style={{ marginLeft: 'auto' }}>
              <div style={{ backgroundColor: '#F4F4F4', height: '20px', width: '20px' }}></div>
            </div>
          </div>
  ...
  // Custom image component with chrome styling
  CustomImage: ({ src, alt, ...props }: { src: string; alt: string; [key: string]: any }) => (
    <div className="my-6">
      <Image
        src={src}
        alt={alt}
        width={800}
        height={400}
        className="rounded-lg shadow-lg border-2 border-chrome/20"
        {...props}
      />
    </div>
  ),
  ...
  h1: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <h1 className="text-futuristic-lg text-white mb-6 mt-8" {...props}>{children}</h1>
  ),
  h2: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <h2 className="text-anime-lg text-chrome mb-4 mt-6" {...props}>{children}</h2>
  ),
  h3: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <h3 className="text-cyberpunk-lg text-white mb-3 mt-5" {...props}>{children}</h3>
  ),

  // Custom paragraph styling
  p: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <p className="text-body-md text-gray-300 mb-4 leading-relaxed" {...props}>{children}</p>
  ),

  // Custom list styling
  ul: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <ul className="text-body-md text-gray-300 mb-4 ml-6 space-y-2" {...props}>{children}</ul>
  ),
  ol: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <ol className="text-body-md text-gray-300 mb-4 ml-6 space-y-2" {...props}>{children}</ol>
  ),
  ...
  blockquote: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <blockquote className="border-l-4 border-chrome pl-6 py-2 my-6 text-body-md text-gray-300 italic" {...props}>
      {children}
    </blockquote>
  ),
  ...
  code: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <code className="bg-gray-800 text-chrome px-2 py-1 rounded text-sm font-mono" {...props}>
      {children}
    </code>
  ),
  pre: ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <pre className="bg-gray-900 text-gray-300 p-4 rounded-lg overflow-x-auto my-6 border border-gray-700" {...props}>
      {children}
    </pre>
  ),
  ...
  a: ({ href, children, ...props }: { href?: string; children?: React.ReactNode; [key: string]: any }) => (
    <a 
      href={href || '#'} 
      className="text-chrome hover:text-white transition-colors duration-300 underline"
